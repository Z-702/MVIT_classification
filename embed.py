...
datafolder = '/data01/zixi/vtk_500_vtk'
outdir = '/data01/zixi/HCP_500_vtk'
os.makedirs(outdir, exist_ok=True)

sub_ids = []
with open('/data01/zixi/tractoembedding_500_subjects/ids_filtered.txt', 'r') as f:
        for line in f:
                sub_ids.append(line.strip())
available_gpus = [0, 3]    #使用合适的空闲GPU           
...
def embed_pipeline(sub_id, gpu_id):
        ...
        tract_vtk = os.path.join(
            ...
            f"{sub_id}",
            "Processed",
            "3T_Diffusion_preproc",
            f"{sub_id}",
            "T1w",
            "Diffusion",
            "ukf_2T_b3000_421a7ad_minGA0p06_minFA0p08_seedFALimit0p10",
            f"{sub_id}_ukf.vtk",
        )
...
if __name__ == "__main__":
    Parallel(n_jobs=len(available_gpus)*2, verbose=1)(
